...
from strategy import Scalp1MStrategy  # We need to adapt this for backtrader
...
class BtScalpStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.core_strategy = Scalp1MStrategy(
            ...
            session_start=datetime.strptime(
                f"{self.p.session_start_hour}:00", "%H:%M"
            ).time(),
            session_end=datetime.strptime(
                f"{self.p.session_end_hour}:00", "%H:%M"
            ).time(),
        )
        self.order = None
    ...
    def open_long(self, price, sl, tp):
        if self.position:  # Prevent new trades if already in a position
            return
        order = self.buy(
            exectype=bt.Order.Limit, price=price, valid=self.data.datetime.datetime(1)
        )
        self.order = order
...
if __name__ == "__main__":
    ...
    cerebro.broker.setcommission(commission=0.0001)  # 0.01% fee
    cerebro.addsizer(bt.sizers.AllInSizer)  # Trade a fixed size
